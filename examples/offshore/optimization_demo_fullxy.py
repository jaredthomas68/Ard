...
yscale = 0.47
rotation = 35.0
boundary_polygon = np.array([[-xb*xscale, -xb*yscale], [xb*xscale, -xb*yscale], [xb*xscale, xb*yscale], [-xb*xscale, xb*yscale]])

def rotate_point(x, y, theta):
    """Rotates a point (x, y) by angle theta (in radians) around the origin."""
...
boundary_polygon = rotate_point(boundary_polygon[:, 0], boundary_polygon[:, 1], math.radians(rotation))
...
modeling_options = {
    "farm": {
        ...
        "boundary": {
            ...
            "turbine_region_assignments": np.zeros(N_turbines, dtype=int),
        },
    },
...
# first the layout
if layout_type == "fullfarm":

    group_layout2aep.add_subsystem("layout_x", om.IndepVarComp("x_turbines", np.random.random((N_turbines,))*max(boundary_polygon[:, 0])*1E3))
    group_layout2aep.add_subsystem("layout_y", om.IndepVarComp("y_turbines", np.random.random((N_turbines,))*max(boundary_polygon[:, 0])*1E3))
...
model.add_subsystem(  # constraints for turbine to boundary proximity
    "boundary_constraint",
    ard.layout.boundary.FarmBoundaryDistancePolygon(
        modeling_options=modeling_options,
    ),
    )
model.connect("layout2aep.x_turbines", "boundary_constraint.x_turbines")
model.connect("layout2aep.y_turbines", "boundary_constraint.y_turbines")
...
test_data = {
    ...
    "boundary_distance": float(
        np.max(prob.get_val("boundary_constraint.boundary_distances", units="km"))
    ),
}
...
if True:
    ...
    prob.model.add_constraint(
        "boundary_constraint.boundary_distances", units="m", upper=0.0
    )
...
optiwindnet.plotting.gplot(prob.model.optiwindnet_coll.graph, ax)
...
scale = 1E3
buf = 100
x = np.append(boundary_polygon[-1, 0]*scale, boundary_polygon[:, 0]*scale)
y = np.append(boundary_polygon[-1, 1]*scale, boundary_polygon[:, 1]*scale)

ax.plot(x, y, "r--")
ax.set(xlim=(-xb*scale*xscale - buf, xb*scale*xscale + buf), ylim=(-xb*scale*yscale - buf, xb*scale*yscale + buf))
plt.show()
